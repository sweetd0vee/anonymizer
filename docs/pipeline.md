...
```mermaid
flowchart TD
  upload["Загрузка DOCX / PDF / TXT / CSV"] --> read["Чтение текста"]
...
| **CSV** | Как TXT: тот же перебор кодировок. Разделители (`,` / `;`) и кавычки не разбираются — замена идёт в тексте файла, колонки сохраняются. | По умолчанию CSV (UTF-8 с BOM, чтобы Excel открыл кириллицу). |
...
- **PDF, TXT и CSV:** замена в плоском тексте, затем сборка выбранного формата. PDF собирается через HTML/Story (PyMuPDF), чтобы кириллица не превращалась в `?`. Исходная вёрстка PDF не копируется. CSV остаётся таблицей: тег ставится внутри ячейки, запятые и точки с запятой между колонками не трогаются.

Формат по умолчанию: PDF → PDF, DOCX → DOCX, CSV → CSV, TXT → DOCX. При скачивании можно выбрать PDF, DOCX, TXT или CSV.

...
# адрес: «г. Москва, ул. Ленина, д. 5, кв. 12» и «Невский пр-т, д. 28, кв. 7»
_STREET_TYPE = (
    r"(?:"
    r"ул\.?|улица|"
    r"пр\.?-?т|пр\.?|проспект|"
...
_CITY = r"(?:(?:г|гор|город|с|село|пос|п|пгт|д|дер|деревня)\.?\s+[А-ЯЁ][\w\-]+,?\s+)?"
_INDEX = r"(?:\d{6},\s*)?"
RX_ADDR = re.compile(
    rf"(?:"
...
def detect_structured(text: str) -> list[Entity]:
    ...
    for m in RX_OGRN.finditer(text):
        if valid_ogrn(m.group(0)) or _ctx_before(text, m.start(), OGRN_CTX, 24):
